...
export default function NewPropertyScreen() {
  ...
  async function handleSave() {
    if (!name.trim()) {
      return Alert.alert('Atenção', 'Nome da propriedade é obrigatório.')
    }
    if (!user) {
    ...
    try {
      ...
      Alert.alert('✓ Propriedade criada', 'Salva localmente. Sincronize quando houver conexão.', [
      ...
      <ScrollView style={styles.container} contentContainerStyle={styles.content}>
          ...
          <View style={styles.tipoRow}>
            ...
            >
              <Ionicons
                name="home"
                size={16}
                color={tipo === 'propria' ? C.primaryDark : C.subtle}
              />
            ...
            >
              <Ionicons
                name="person"
                size={16}
                color={tipo === 'cliente' ? '#a8451d' : C.subtle}
              />
        ...
        >
          {saving ? (
            ...
            <Text style={styles.saveBtnText}>💾 Salvar propriedade</Text>
          )}
        </TouchableOpacity>
