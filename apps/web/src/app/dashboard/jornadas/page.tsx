...
import { jornadasApi, tenantUsersApi } from '@/lib/api'
...
// Gera lista de meses disponíveis (12 meses para trás)
...
export default function JornadasPage() {
  ...
  const { data: allJornadas, isLoading } = useQuery({
    queryKey: ['jornadas-view', mes, vendedorId],
    queryFn: () =>
      jornadasApi
        .list({
          mes: mes || undefined,
          collaborator_id: vendedorId || undefined,
        })
        .then((r) => r.data),
  })
  ...
  const jornadas = allJornadas ?? []

  const totalKm = useMemo(() => jornadas.reduce((a, j) => a + (j.total_distance_km ?? 0), 0), [jornadas])
  const totalJornadas = jornadas.length
  const totalTravelMin = useMemo(() => jornadas.reduce((a, j) => a + (j.total_travel_minutes ?? 0), 0), [jornadas])

  const subtitleParts = [
    `${totalJornadas} jornada${totalJornadas !== 1 ? 's' : ''}`,
    `${totalKm.toFixed(1)} km total`,
    `${formatMin(totalTravelMin)} em deslocamento`,
  ]
      ...
      <div style={{ padding: '24px 32px 0', display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16 }}>
        ...
        <Card>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <div>
              <div style={{ fontSize: '0.7rem', color: 'var(--color-text-subtle)', textTransform: 'uppercase', letterSpacing: '0.05em', fontWeight: 600 }}>Média por Jornada</div>
              <div style={{ fontSize: '1.25rem', fontWeight: 700 }}>
                {totalJornadas > 0 ? `${(totalKm / totalJornadas).toFixed(1)} km` : '—'}
              </div>
            </div>
          </div>
        </Card>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ borderBottom: '1px solid var(--color-border)' }}>
                  {['Data', 'Vendedor', 'Objetivo', 'Origem', 'Destino / Cliente', 'Distância', 'Tempo', 'Veículo', ''].map((h) => (
                    <th key={h} style={{
                      padding: '12px 14px',
                      textAlign: 'left',
                      fontSize: '0.72rem',
              ...
              <tbody>
                ...
                {jornadas.map((j, i) => {
                  const origin = j.origin_name ?? '—'
                  const destination = j.client_name ?? '—'
                  const odometerKm =
                    j.km_odometer_start != null && j.km_odometer_end != null
                      ? j.km_odometer_end - j.km_odometer_start
                      : null
                  ...
                  return (
                    <tr
                      key={j.id}
                      style={{
                        borderBottom: i < jornadas.length - 1 ? '1px solid var(--color-border)' : 'none',
                    ...
                    >
                      <td style={{ padding: '12px 14px', fontSize: '0.85rem', color: 'var(--color-text-muted)', whiteSpace: 'nowrap' }}>
                        {format(parseISO(j.date), "dd 'de' MMM", { locale: ptBR })}
                        {j.started_at && (
                          <div style={{ fontSize: '0.72rem', color: 'var(--color-text-subtle)' }}>
                            {format(new Date(j.started_at), 'HH:mm')}
